...
**Retested 2026-07-23** with `astro@7.1.3`, `vite@8.1.5`, `@tailwindcss/vite@4.3.3` (all latest). Still broken, though the symptom shifted: the compiled `PageLayout*.css` is now 142 KB with 23 `@media` queries (up from the earlier 30 KB/theme-only profile), so a size or media-query-count check alone would misread this build as fixed. The queries present are all non-breakpoint (`hover`, `prefers-reduced-motion`, `print`, `pointer`, `forced-colors`) — grepping for the actual responsive utility prefixes shows the real signal: `md:`, `lg:`, `sm:`, `xl:` all occur **zero** times in the astro 7 build's CSS, versus 49/25/31/2 respectively in the astro 6 baseline build from the same source tree. Verify with `python3 -c "print(open('dist/_astro/PageLayout*.css').read().count('md\\\\:'))"` (or grep for the literal `md\:` substring) rather than counting `@media` occurrences.
